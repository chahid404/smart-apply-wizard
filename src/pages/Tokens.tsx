    ...
    <div className="container max-w-6xl py-6 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {TokenPackages.map((pkg) => (
          <Card key={pkg.tokens} className={`p-6 relative ${pkg.popular ? "border-2 border-teal" : ""}`}>
            {pkg.popular && (
              <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                <span className="bg-teal text-white px-3 py-1 rounded-full text-sm">Most Popular</span>
              </div>
            )}
            <div className="space-y-4">
              <div className="text-center space-y-1">
                ...
                <p className="text-gray-600 text-sm">${(pkg.price / pkg.tokens).toFixed(2)} per token</p>
              </div>
              <Button className="w-full gap-2" onClick={() => handlePurchase(pkg.tokens, pkg.price)}>
